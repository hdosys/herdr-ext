---
name: triage
description: Triage open Herdr Extended GitHub issues into a concise decision-first Markdown table. Use when the user asks which distribution issues need attention or wants issue priority/recommendation lights.
---

# Herdr Extended issue triage

Use this skill only inside the `hdosys/herdr-ext` repository.

When the user says `triage`, inspect open GitHub issues for `hdosys/herdr-ext` and return a concise Markdown table. Query official `herdrdev/herdr` only when the user explicitly requests upstream triage. Prefer GitHub MCP tools when available. If they are unavailable, use `gh issue list` / `gh issue view` only when authenticated access is already configured.

Use this table shape:

| Light | Recommendation | Issue | Age | Reactions | Why |
|---|---|---|---:|---:|---|
| 🔴 | fix now | [#123](https://github.com/hdosys/herdr-ext/issues/123) | 18d | 5 | user-visible regression |
| 🟡 | queue | [#124](https://github.com/hdosys/herdr-ext/issues/124) | 42d | 2 | useful but not blocking |
| 🔵 | defer | [#125](https://github.com/hdosys/herdr-ext/issues/125) | 7d | 0 | cosmetic polish |
