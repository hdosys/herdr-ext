...
Command producers can make an entry clickable by wrapping the complete last-line
label in one OSC 8 hyperlink. No configuration change is required. Emit actual
ESC (`0x1b`) and BEL (`0x07`) bytes, not the printable escape notation below:

```text
ESC]8;;https://chatgpt.com/codex/cloud/settings/analytics BELCodex 42%ESC]8;;BEL
```

There is no space before either BEL in the emitted bytes. For example, a Python
producer can print:

```python
print("\x1b]8;;https://chatgpt.com/codex/cloud/settings/analytics\x07Codex 42%\x1b]8;;\x07")
```

Both BEL and ST (`ESC` followed by `\`) are accepted terminators. Emit only one
complete link around the whole label. Plain text continues to work. Invalid,
incomplete, non-HTTP(S), or credential-bearing authority URLs display as plain
text without activation. Labels retain the existing 80-character limit and control
sanitization; the complete output line, including the URL, must fit in 4096 bytes.
...
**F10** to open the menu, select the status label with **Up/Down**, and press
...
remote Sandbox server. Both server and client need clickable-status support for
activation. Older servers strip OSC 8 to plain labels; older compatible clients
ignore the optional link metadata and still display the labels.
