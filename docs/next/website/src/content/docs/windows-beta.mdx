...
Install native Windows builds by downloading the newest installer from the [Herdr Extended releases](https://github.com/hdosys/herdr-ext/releases). The per-user NSIS setup requires no administrator access, registers **Herdr Extended** in Installed Apps, and puts `%LOCALAPPDATA%\Programs\Herdr\bin` on the user `PATH`. One stable launcher selects immutable runtimes under `%LOCALAPPDATA%\Programs\Herdr\runtime` so active sessions can finish before a newer runtime is activated.

The installed Windows distribution owns its fork CalVer update feed and cannot be redirected to official Herdr or another channel through config.
...
Every supported client can attach to x86_64 and ARM64 Windows hosts. Ordinary attachment and saved machines reuse an endpoint-compatible binary from `PATH` or `%USERPROFILE%\.herdr\remote\herdr.exe`; a version difference alone does not require replacement. Explicit provisioning instead requires the selected deployment's exact runtime identity and validates the complete portable ZIP before any approved server stop. It never runs the managed installer or changes remote `PATH`. Saved reconnects do not install or restart a server, and retain their explicitly selected session. The Windows OpenSSH default shell must be `cmd.exe` or PowerShell 7, and the server starts in the SSH user's existing active desktop session so it survives SSH closure. Windows PowerShell 5.1, an unknown shell, or an ambiguous desktop session fails closed. Windows remote hosts do not support live handoff.

Windows targets must also advertise `windows_remote_host` support. A client-only Windows binary is not sufficient to host the SSH bridge.

Windows updates run through the managed setup asset. Existing sessions keep their current immutable runtime, and future launches switch after active leases exit. Live handoff is Unix-only.
